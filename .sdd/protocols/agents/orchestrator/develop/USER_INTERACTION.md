...
- If it has unresolved follow-up items, present them to the user via `ask_user`:
...
- **No interactive `ask_user` calls** are made for configuration variables. All configuration is derived from the frontmatter.
...
- Before making the *first* modification to `.sdd/instructions/${PROTOCOL}.md`, you MUST ask the user using `ask_user`:
...
- Upon detecting the canonical `Cycle binding mismatch` error, perform the following steps:
  1. Notify the user using `ask_user` with the message: *"A cycle binding mismatch has been detected. This may occur when the protocol state and cycle state are inconsistent. I will attempt to reindex the active cycle to resolve this issue. Proceed?"*
  2. Wait for user confirmation (clear "yes").
